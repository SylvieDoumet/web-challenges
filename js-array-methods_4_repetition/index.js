...
const onlyTitles = cards.map((card) => card.title);

const titlesWithDuration = cards.map((card) => card.title && card.title.number);

const timePerServing = cards.map((card) => card.servings > 0);
...
const allTitlesInOneString = cards.map((card) => card.title).join(", ");
...
const recipesThatOnlyTake60minutesOrLess = cards.filter(
  (card) => card.duration < 60
);

const allRecipesWithMoreThan2Servings = cards.filter(
  (card) => card.servings > 2
);

const allRecipesWithTitlesLongerThan12Characters = cards.filter(
  (card) => card.title.length > 12
);
